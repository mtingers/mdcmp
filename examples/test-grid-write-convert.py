...
from mdcmp.composer import Composer
...
def test1():
    ...
    grid.save('data/mdc/misc/test1.mdc', velocity_jitter=7, humanize_jitter=False)
    # Convert the test.mdc file to test.midi file
    converter = Converter(tempo=115)
    converter.convert("data/mdc/misc/test1.mdc")
    converter.save("midi/test1.midi")
...
def test2():
    ...
    grid.copy_to_end(bars=[0, 1, 2, 3, 4, 5, 6, 7], tracks=[0, 1, 2, 3], count=20, strict=False)
    # grid.save('data/mdc/misc/test.mdc', velocity_jitter=7, humanize_jitter=True)
    grid.save('data/mdc/misc/test2.mdc', velocity_jitter=7, humanize_jitter=False)
    # Convert the test.mdc file to test.midi file
    converter = Converter(tempo=115)
    converter.convert("data/mdc/misc/test2.mdc")
    converter.save("midi/test2.midi")


def test_composer1():
    composer = Composer(133)
    composer.load_mdc_bank("data/mdc")
    composer.convert_mdc("misc.test1")
    composer.convert_mdc("misc.test2")
    composer.save("midi/composertest1.midi")
